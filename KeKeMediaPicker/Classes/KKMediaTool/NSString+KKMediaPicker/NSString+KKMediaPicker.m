...
@implementation NSString (KKMediaPicker)
...
+ (BOOL)kkmp_isStringNotEmpty:(nullable id)string{
    if (string && [string isKindOfClass:[NSString class]] && [[string kkmp_trimLeftAndRightSpace] length]>0) {
        return YES;
    }
    else{
        return NO;
...
+ (BOOL)kkmp_isStringEmpty:(nullable id)string{
    return ![NSString kkmp_isStringNotEmpty:string];
}
...
+ (BOOL)kkmp_isLocalFilePath:(nullable id)string{
    if ([NSString kkmp_isStringEmpty:string]) {
        return NO;
    } else {
...
- (NSString *)kkmp_hiddenPhoneNum{
    
    if ([self kkmp_isMobilePhoneNumber]) {
...
- (CGSize)kkmp_sizeWithFont:(nullable UIFont *)font
    ...
    return [self kkmp_sizeWithFont:font maxSize:CGSizeMake(width, CGFLOAT_MAX) lineBreakMode:lineBreakMode];
}
...
- (CGSize)kkmp_sizeWithFont:(nullable UIFont *)font
    ...
    return [self kkmp_sizeWithFont:font maxSize:CGSizeMake(width, CGFLOAT_MAX) lineBreakMode:NSLineBreakByWordWrapping];
}
...
- (CGSize)kkmp_sizeWithFont:(nullable UIFont *)font
    ...
    return [self kkmp_sizeWithFont:font maxSize:size inset:UIEdgeInsetsMake(0, 0, 0, 0) lineBreakMode:NSLineBreakByWordWrapping];
}
...
- (CGSize)kkmp_sizeWithFont:(nullable UIFont *)font
    ...
    return [self kkmp_sizeWithFont:font maxSize:size inset:UIEdgeInsetsMake(0, 0, 0, 0) lineBreakMode:lineBreakMode];
}
...
- (CGSize)kkmp_sizeWithFont:(nullable UIFont *)font
    ...
    return [self kkmp_sizeWithFont:font maxSize:CGSizeMake(width, CGFLOAT_MAX) inset:inset lineBreakMode:NSLineBreakByWordWrapping];
}
...
- (CGSize)kkmp_sizeWithFont:(nullable UIFont *)font
    ...
    return [self kkmp_sizeWithFont:font maxSize:CGSizeMake(width, CGFLOAT_MAX) inset:inset lineBreakMode:lineBreakMode];
}
...
+ (nullable NSString*)kkmp_chineseUperTextFromDigitalString:(nullable NSString*)aDigitalString{
    ...
    if( ![aDigitalString kkmp_isInteger] && ![aDigitalString kkmp_isFloat]){
        return @"";
    }
    ...
    if ([aDigitalString kkmp_isFloat]) {
    ...
    else if ([aDigitalString kkmp_isInteger]){
        returnString = [returnString stringByAppendingFormat:@"%@",@"整"];
    }
    else{
        returnString = @"";
...
/**
 ...
 @return 结果
 */
+ (nullable NSString*)kkmp_subStringForNSUTF8StringEncodingWithSize:(NSInteger)size
                                                           string:(nullable NSString*)string{
    
    NSString *tempString = [NSString stringWithString:string];
    
    NSInteger tempStringSize = [NSString kkmp_sizeOfStringForNSUTF8StringEncoding:tempString];
    ...
    if (size>tempStringSize/2) {
        ...
        while (1) {
            if ([NSString kkmp_sizeOfStringForNSUTF8StringEncoding:tempString]<=size) {
                break;
            }
            else{
                index = index -1;
...
+ (nullable NSString*)kkmp_subStringForNSUnicodeStringEncodingWithSize:(NSInteger)size
    ...
    NSInteger tempStringSize = [NSString kkmp_sizeOfStringForNSUnicodeStringEncoding:tempString];
    if (tempStringSize <= size) {
        return tempString;
    }
    ...
    if (size>tempStringSize/2) {
        NSInteger index = [tempString length];
        while (1) {
            if ([NSString kkmp_sizeOfStringForNSUnicodeStringEncoding:tempString]<=size) {
                break;
            }
            else{
                index = index -1;
...
/**
 ...
 @return 截取的子字符串
 */
- (NSString *)kkmp_substringFromIndex_Safe:(NSUInteger)from {
    if (from > self.length ) {
        return nil;
    }
...
/**
 ...
 @return 返回截取的字符串
 */
- (NSString *)kkmp_substringToIndex_Safe:(NSUInteger)to {
    if (to > self.length ) {
        return nil;
    }
...
/**
 ...
 @return 返回搜索到的字符串 范围
 */
- (NSRange)kkmp_rangeOfString_Safe:(NSString *)searchString
...
/**
 ...
 @return 返回截取的字符串
 */
- (NSString *)kkmp_substringWithRange_Safe:(NSRange)range {
