...
- (void)initUI{
    ...
    CGRect backFrame = CGRectMake(0, 0, self.backgroundView.frame.size.width, 1);
...
- (void)beginHide{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.table.frame = CGRectMake(0, (-1)*self.table.frame.size.height, self.frame.size.width, self.table.frame.size.height);
        self.tableBackView.frame = CGRectMake(0, (-1)*self.tableBackView.frame.size.height, self.frame.size.width, self.tableBackView.frame.size.height);
...
- (void)beginShow{
    ...
    self.tableBackView.frame = CGRectMake(0, (-1)*self.tableBackView.frame.size.height, self.frame.size.width, self.tableBackView.frame.size.height);
    [UIView animateWithDuration:0.25 animations:^{
        self.table.frame = CGRectMake(0, 0, self.frame.size.width, self.table.frame.size.height);
        self.tableBackView.frame = CGRectMake(0, 0, self.frame.size.width, self.tableBackView.frame.size.height);
...
//        [backView setCornerRadius:10 type:KKCornerRadiusType_LeftBottom | KKCornerRadiusType_RightBottom];
        UIBezierPath* rounded = [UIBezierPath bezierPathWithRoundedRect:backView.bounds byRoundingCorners:UIRectCornerBottomLeft|UIRectCornerBottomRight cornerRadii:CGSizeMake(10, 10)];
...
//        [backView setCornerRadius:0 type:KKCornerRadiusType_LeftBottom | KKCornerRadiusType_RightBottom];
        UIBezierPath* rounded = [UIBezierPath bezierPathWithRoundedRect:backView.bounds byRoundingCorners:UIRectCornerBottomLeft|UIRectCornerBottomRight cornerRadii:CGSizeMake(0, 0)];
        CAShapeLayer* shape = [[CAShapeLayer alloc] init];
        [shape setPath:rounded.CGPath];
        backView.layer.mask = shape;
    }
