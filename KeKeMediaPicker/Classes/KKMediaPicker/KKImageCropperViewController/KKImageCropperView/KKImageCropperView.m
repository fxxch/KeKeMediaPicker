...
//    aHeight *= _maskViewZoomScale;
    ...
    UIImage *image = [_image kkmp_cropImageWithX:aX y:aY width:aWidth height:aHeight];
    if (_cropRatio == 0) {
        image = [image kkmp_resizeToWidth:_cropSize.width/screenScale height:_cropSize.height/screenScale];
    } else {
        image = [image kkmp_resizeToWidth:image.size.width/screenScale height:image.size.height/screenScale];
    }
