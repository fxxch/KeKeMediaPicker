...
- (void)initBottomButtons{
    UIImage *chaImage = [NSBundle kkmp_imageInBundle:@"KKCameraCapture.bundle" imageName:@"chexiao"];
    UIButton *chaButton = [[UIButton alloc] initWithFrame:CGRectMake(15, UIWindow.kkmp_screenHeight-UIWindow.kkmp_safeAreaBottomHeight-90-30, 90, 90)];
    ...
    self.chaButton.layer.cornerRadius = self.chaButton.frame.size.width/2.0;
    self.chaButton.layer.masksToBounds = YES;

    UIImage *gouImage = [NSBundle kkmp_imageInBundle:@"KKCameraCapture.bundle" imageName:@"gou"];
    UIButton *gouButton = [[UIButton alloc] initWithFrame:CGRectMake(UIWindow.kkmp_screenWidth-15-90, UIWindow.kkmp_screenHeight-UIWindow.kkmp_safeAreaBottomHeight-90-30, 90, 90)];
    ...
    self.goButton.layer.cornerRadius = self.goButton.frame.size.width/2.0;
    self.goButton.layer.masksToBounds = YES;

    CGPoint centerPoint1 = self.chaButton.center;
    centerPoint1.x = UIWindow.kkmp_screenWidth/2.0;
    self.chaButton.center = centerPoint1;
    
    CGPoint centerPoint2 = self.goButton.center;
    centerPoint2.x = UIWindow.kkmp_screenWidth/2.0;
    self.goButton.center = centerPoint2;
}

- (void)bottomButtonsAnimated{
    [UIView animateWithDuration:0.5 animations:^{
        CGPoint centerPoint1 = self.chaButton.center;
        centerPoint1.x = 30+self.chaButton.frame.size.width/2.0;
        self.chaButton.center = centerPoint1;
        
        CGPoint centerPoint2 = self.goButton.center;
        centerPoint2.x = UIWindow.kkmp_screenWidth-30-self.goButton.frame.size.width/2.0;
        self.goButton.center = centerPoint2;
    }];
}
...
#pragma mark ==================================================
- (void)initVideoUI{
    CGRect frame = CGRectMake(0, 0, UIWindow.kkmp_screenWidth, UIWindow.kkmp_screenHeight);
    //判断是否是iphoneX
    if ([UIWindow kkmp_currentKeyWindow].safeAreaInsets.bottom > 0.0) {
        frame = CGRectMake(0, UIWindow.kkmp_statusBarHeight, UIWindow.kkmp_screenWidth, UIWindow.kkmp_screenHeight-UIWindow.kkmp_statusBarHeight-UIWindow.kkmp_safeAreaBottomHeight);
    }
...
#pragma clang diagnostic pop
    ...
    UIImage *playImage = [NSBundle kkmp_imageInBundle:@"KKCameraCapture.bundle" imageName:@"play"];
...
- (void)expotVideoToMP4WithFileURL:(NSURL *)fileURL{
    ...
    if (tmpAry.count>0) {
        ...
        [KKMediaPickerWatingView hideForView:self.view animation:YES];
        return;
    }
